/**
 * Tests for MCPClient.validateDossier + fetchValidateReport.
 *
...
const chatPromptMissingDoc: { event: string; data: ValidateChatPrompt } = {
  ...
  data: {
    ...
    submit_label: 'Upload',
    context: { run_id: RUN_ID, thematic_key: 'category_d', iteration_index: 1 },
  },
};
...
const chatPromptClassif: { event: string; data: ValidateChatPrompt } = {
  ...
  data: {
    ...
    prompt_type: 'form',
    title: 'Which document type ?',
    description: 'doc-unknown.pdf',
    fields: [
      {
        type: 'select',
        name: 'thematic_key',
...
describe('MCPClient.validateDossier — single stream', () => {
  ...
  test('happy path: dispatches every event and terminates on validate:complete', async () => {
    ...
    const result = await client.validateDossier(
  ...
  test('first request body wraps the input in a JSON-RPC tools/call envelope', async () => {
    ...
    expect(parsed.params.name).toBe('deposium_validate_dossier');
...
describe('MCPClient.validateDossier — Mode A resume (re-classify)', () => {
  const originalFetch = globalThis.fetch;
...
describe('MCPClient.validateDossier — Mode B resume (structured response)', () => {
  const originalFetch = globalThis.fetch;
    ...
    const result = await client.validateDossier(
      { dossier_id: 'd-1', level: 1, on_ambiguity: 'prompt' },
      {
        ...
        onChatPrompt: async () => ({
          ...
          hitlResponse: {
            correlation_id: CORR_ID,
            fields: { thematic_key: 'category_a' },
          },
        }),
      }
    );
...
describe('MCPClient.validateDossier — error propagation', () => {
  const originalFetch = globalThis.fetch;
